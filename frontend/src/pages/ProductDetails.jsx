...
const ProductDetails = () => {
  ...
  if (!product) return <p className="text-center text-muted py-20">Product not found.</p>;
    ...
    <div className="max-w-7xl mx-auto px-6 py-12 grid md:grid-cols-2 gap-12">
      ...
      <div>
        ...
        <div className="flex items-center gap-3 mb-6">
          {hasDiscount ? (
            <>
              <span className="text-2xl text-crimson font-medium">₹{product.discountPrice}</span>
              <span className="text-muted line-through">₹{product.price}</span>
            </>
          ) : (
            <span className="text-2xl text-ink font-medium">₹{product.price}</span>
          )}
        </div>
